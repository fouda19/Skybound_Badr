    ...
    public:
        void update(World *world)
        {
            ...
            for (auto entity : world->getEntities())
            {
                ...
                if (light)
                {
                    world->lights[world->light_count].type = light->kind;                                                            
                    world->lights[world->light_count].position = glm::vec3(entity->getLocalToWorldMatrix() * glm::vec4(0, 0, 0, 1)); // the position of the light in the world space
